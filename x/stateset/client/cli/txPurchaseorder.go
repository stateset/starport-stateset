...
func GetCmdCreatePurchaseorder(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "create-purchaseorder [number] [name] [type] [status] [startDate] [endDate] [total] [hash]",
		Short: "Creates a new purchaseorder",
      ...
      argsHash := string(args[7])
			...
			msg := types.NewMsgCreatePurchaseorder(cliCtx.GetFromAddress(), argsNumber, argsName, argsType, argsStatus, argsStartDate, argsEndDate, argsTotal, argsHash)
