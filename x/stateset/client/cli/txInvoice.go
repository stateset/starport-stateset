...
func GetCmdCreateInvoice(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "create-invoice [name] [reason] [amountDue] [amountPaid] [total]",
		Short: "Creates a new invoice",
      ...
      argsTotal := string(args[4])
			...
			msg := types.NewMsgCreateInvoice(cliCtx.GetFromAddress(), argsName, argsReason, argsAmountDue, argsAmountPaid, argsTotal)
